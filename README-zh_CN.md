...
> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=deepcode-cli)** 是一个全模态、OpenAI 兼容的 AI 推理平台 —— 把 `BASE_URL` 指向 `https://api.atlascloud.ai/v1` 即可作为 Deep Code 的即插即用后端，通过单一 API 接入 DeepSeek、Claude、Qwen、GLM、Kimi、MiniMax 等模型，无需逐家厂商对接。预算友好的 [coding plan](https://www.atlascloud.ai/console/coding-plan)。
...
`deepseek-ai/deepseek-v4-pro` 是推理模型，需要给足输出预算（≥ 512 tokens），否则可能在思维链处停止、返回空 content。也可以使用 Claude、Qwen 等其他模型，API Key 在 [Atlas Cloud 控制台](https://www.atlascloud.ai/console/coding-plan) 获取。

<details>
<summary>全部 Atlas Cloud 对话模型（59 个，OpenAI 兼容）</summary>
