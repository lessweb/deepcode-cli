...
test("createSession appends default system prompts in prefix-cache-friendly order", async () => {
  ...
  assert.doesNotMatch(systemContents[0] ?? "", /Current LLM model is test-model/);
  ...
  assert.doesNotMatch(systemContents[1] ?? "", /Current LLM model is test-model/);
  assert.match(systemContents[2] ?? "", /# Local Workspace Environment/);
  assert.match(systemContents[2] ?? "", /Current LLM model is test-model/);
